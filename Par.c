...
// This function takes the process ID and returns the binary label. This label will be used during process communication in the HyperQuickSort Algorithm
int * GetLabel( int ID, int d ) ;
...
int * HyperQuickSort( int * A, int l, int h, int * label, int k, int * recv_data, int * G, int * send_data ) ;
...
{
    double t ;
    int n, p, *A, * label, i, * tmp, * Buffer, * final, * C, h, counter = 0, size, tmpp, total = 0, * recv_data, * G, * send_data ;
    ...
    // The number of processes must be a power of 2
        ...
        // Get binary label of each process
        label = GetLabel(ID, d) ;
        ...
        if( ID == MASTER )
            t = MPI_Wtime() ;
        C = HyperQuickSort(A, 0, size-1, label, 0, recv_data, G,send_data) ;
...
{
    int i, j ;
    int * label = (int*) malloc(sizeof(int)*d) ;
    for( i = d-1 ; ID > 0 ; i-- ){
        label[i] = ID%2 ;
        ID = ID / 2 ;
    }
...
int * HyperQuickSort( int * A, int l, int h, int * label, int k, int * recv_data, int * G, int * send_data )
{
    ...
    return HyperQuickSort( G, 0, z-1, label, k+1, recv_data, G, send_data ) ;
}
